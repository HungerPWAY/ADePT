        ...
        try:
            if not isinstance(peft_config, PromptLearningConfig):
                ...
                if peft_config.peft_type == PeftType.PREFIX_TUNING:
                    ...
                    scaling = self.prompt_encoder[self.active_adapter].scaling
                    #inputs_embeds += scaling * (lora_embedding_A @ lora_embedding_B).repeat(batch_size, 1, 1)
                    #print(torch.nn.functional.relu(inputs_embeds @ lora_embedding_A ) @ lora_embedding_B)
                    inputs_embeds =inputs_embeds +  torch.nn.functional.relu(inputs_embeds @ lora_embedding_A ) @ lora_embedding_B
                    batch_size = inputs_embeds.shape[0]
                    prompts = self.get_prompt(batch_size=batch_size)
                    prompts = prompts.to(inputs_embeds.dtype)
                    ...
                    '''
                    import numpy as np
                    if os.path.exists('/data/nas/tangpwei/Adept/results.npz'):
                        # 加载现有的 .npz 文件
                        existing_data = np.load('/data/nas/tangpwei/Adept/results.npz')
                        existing_result1 = existing_data['result1']  # 形状为 [10, 768]
                        existing_result2 = existing_data['result2']  # 形状为 [10, 768]
                    else:
                        # 如果文件不存在，创建空的数组
                        existing_result1 = np.empty((0, 768))  # 形状为 [0, 768]
                        existing_result2 = np.empty((0, 768))  # 形状为 [0, 768]
                    new_result1 = (torch.abs(inputs_embeds.reshape(-1,768))).detach().cpu().numpy()
                    #print(3343334244,new_result1.shape)
                    new_result2 = torch.abs(torch.nn.functional.relu(temp) @ lora_embedding_B + lora_embedding_b).reshape(-1,768)
                    new_result2 = new_result2.detach().cpu().numpy()
                    #print(3423424,new_result2.shape)
                    # 将新的结果与已存储的结果进行连接
                    print(existing_result1.shape)
                    print(existing_result2.shape)
                    print(new_result1.shape)
                    print(new_result2.shape)
                    concatenated_result1 = np.concatenate((existing_result1, new_result1), axis=0)
                    concatenated_result2 = np.concatenate((existing_result2, new_result2), axis=0)

                    # 保存连接后的结果到新的 .npz 文件
                    np.savez_compressed('/data/nas/tangpwei/Adept/results.npz', result1=concatenated_result1, result2=concatenated_result2)

                    # 打印连接后的结果形状
                    r1 = np.mean(concatenated_result1,axis=0)
                    r2 = np.mean(concatenated_result2,axis=0)
                    print(np.mean(r1))
                    print(np.mean(r2))
                    print(np.std(r1))
                    print(np.std(r2))
                    print("Concatenated Result 1 Shape:", concatenated_result1.shape)  # 输出: (1, 768) 或 (11, 768)
                    print("Concatenated Result 2 Shape:", concatenated_result2.shape)  # 输出: (1, 768) 或 (11, 768)
                    '''
