...
@dataclass
class PromptTuningLoRAXABConfig(PromptLearningConfig):
    """
    ...
    def __post_init__(self):
        self.peft_type = PeftType.PROMPT_TUNING_LORAXAB


class PromptEmbeddingLoRAXAB(torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def __init__(self, config, word_embeddings):
        ...
        self.lora_embedding_c = nn.Parameter(torch.zeros((128)), requires_grad=True)
        self.lora_embedding_d = nn.Parameter(torch.zeros((config.token_dim)), requires_grad=True)
        self.lora_embedding_C = nn.Parameter(torch.zeros((config.token_dim, 128)), requires_grad=True)
        self.lora_embedding_D = nn.Parameter(torch.zeros((128, config.token_dim)), requires_grad=True)
