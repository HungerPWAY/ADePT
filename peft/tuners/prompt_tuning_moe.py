...
class PromptEmbeddingLoRA(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config, word_embeddings):
        ...
        self.lora_embedding_C = nn.Parameter(torch.zeros((config.token_dim, config.r)), requires_grad=True)
        self.lora_embedding_D = nn.Parameter(torch.zeros((config.r, config.token_dim)), requires_grad=True)
        self.lora_embedding_E = nn.Parameter(torch.zeros((config.token_dim, config.r)), requires_grad=True)
        self.lora_embedding_F = nn.Parameter(torch.zeros((config.r, config.token_dim)), requires_grad=True)
        if config.r == 0:
            self.scaling = 0
        else:
        ...
        nn.init.kaiming_uniform_(self.lora_embedding_E, a=math.sqrt(5))
        nn.init.zeros_(self.lora_embedding_F)
